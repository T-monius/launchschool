...
$(function() {
  $('#add_item').on('click', function() {
    var newId = inventory.newId();
    inventory.addItem();
    var newItem = inventory.collection.find((item) => item.id === newId);
    $('#inventory').append(inventory.template(newItem));
  });
  ...
  $('table').on('click', "a[class='delete']", function(clickEvent) {
    ...
    $deleteAnchor.parents('tr').remove();
  })
});
