...
let CarGallery = (function() {
  ...
  function removeDuplicates(list) {
    ...
    list.forEach( (value) => {
      if (!cleanList.includes(value)) {cleanList.push(value)}
    });
  ...
  return {
    ...
    populateFilterForm: function() {
      allInfoFields = this.retrieveInfoFields();
      $('main header').append(this.filterFormTemplate(allInfoFields));
    },
    ...
    setFilterHandlers: function() {
      ...
      $form.on('input', '#make_select', function(inputEvent) {
        let $makeSelectBox = $(inputEvent.target);
        let filteredModels = removeDuplicates(self.filterModelsByMake($makeSelectBox.val()));

        self.restrictModelsToMake(filteredModels);
      });
    },
    filterModelsByMake: function(inputMake) {
      return cars.filter( (car) => inputMake === car.make )
                 .map( (car) => car.model );
    },
    restrictModelsToMake: function(models) {
      $('#model_select option').each( (_, modelOption) => {
          if(!models.includes($(modelOption).val())) { $(modelOption).remove() }
      });
    },
